...
> Current extension build: **v1.3.44** — after v1.3.43 shipped, a real full-semester export still came back with a handful of Ed-image codes missing even though the fixed page-discovery logic was opening the exact right thread every time; the codes were sitting right there in the screenshot, but OCR itself was dropping them. Comparing the OCR passes' own internal logs against the real images showed why: on a couple of FIT2102/FIT2109 attendance tables, one whole row - the session label and its code together, not just the code cell - vanished from the main recognition pass while every other row in the same image read correctly. The rescue logic that was supposed to catch this only ran a second, more careful pass when the number of codes found came up short of the number of rows found, but a row that disappears in one piece keeps both counts moving together, so that comparison never had anything to trip on and the rescue passes never fired. This build removes that count comparison and instead runs the extra passes based only on the image's own shape - a second full-image reading whenever the image is tall enough to plausibly hold more than one row, and a dedicated crop of just the code column whenever the image is shaped like a real table - so a fully-dropped row gets a genuine second look regardless of whether the running totals happened to still look balanced. This has not yet been confirmed against a real re-run (OCR only runs inside the browser extension itself), so treat it as a real fix worth testing rather than a proven one until the next real export is checked. (v1.3.43 fixed three separate causes of FIT2102/FIT2109/FIT3162 codes never being found at all: Ed's lazy-loaded discussion list, course links being tagged with every enrolled unit's code instead of just their own, and Moodle's "My units" page needing far longer than the old wait budget to render; v1.3.42 added the debug logging that made all three visible instead of guesswork; v1.3.41 fixed the Ed fallback's flat 2-thread cap, a related but separate problem from the lazy-loading one above; v1.3.40 sent an opened Gmail message's screenshot-based code tables to OCR; v1.3.39 filled in the weeks Attendance's own UI can no longer show at all; v1.3.38 added the on-screen requested-vs-actual date range and fixed a debug-log write race; v1.3.37 fixed a flat Gmail thread cap sized for a single week's scan.)
