...
> Current extension build: **v1.3.43** — the v1.3.42 debug logging paid off: a real export run finally showed exactly where FIT2102 and FIT3162 were failing, and all three turned out to be real, separate bugs rather than one. First, Ed's discussion list only renders its ~30 most recent threads until someone clicks "Load more" - FIT2102's Week 1-6 attendance threads were never missing from the matching logic, they simply did not exist in the scanned page at all, so this build clicks that button (bounded, confirmed against a real course going from 24 to 278 threads and surfacing every week from 1 to 8) before reading the page. Second, matching an Ed/Moodle course link to its course code was reading a climbed-up chunk of nearby page text that, on the real dashboard, turned out to be the whole sidebar list of every enrolled unit concatenated together - so every course link was tagged with every requested code instead of just its own; this build matches the link's own label first and only falls back to that wider context when the label alone names no course. Third, Moodle's "My units" page took a measured 17.4 seconds just to render its course cards at all, already past the old 15-second wait budget, which is why the Moodle fallback never found a single course to open; the wait budget is now 45 seconds. (v1.3.42 added the debug logging that made all three of these visible instead of guesswork; v1.3.41 fixed the Ed fallback's flat 2-thread cap, a related but separate problem from the lazy-loading one above; v1.3.40 sent an opened Gmail message's screenshot-based code tables to OCR; v1.3.39 filled in the weeks Attendance's own UI can no longer show at all; v1.3.38 added the on-screen requested-vs-actual date range and fixed a debug-log write race; v1.3.37 fixed a flat Gmail thread cap sized for a single week's scan.)
