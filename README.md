...
> Current extension build: **v1.3.45** — v1.3.44 shipped and a real re-export confirmed it worked: three of the four previously-missing FIT2102/FIT2109 codes came back correctly. The remaining one (FIT2102 Workshop 01, Aug 18) turned out to be a different bug, found by opening the real Ed post and testing the exact same image through the real OCR code by hand. That row is published as its own tiny 841×42-pixel strip, separate from the rest of the table, and even after the extension's own upscaling Tesseract read the code cell as visible garbage ("CcQlTe"+ an accented character) while a dedicated, whitelisted crop of just that code column correctly recovered a real 5-character reading from the same spot. The problem was that the garbled reading, once uppercased, still happened to contain a run of 5 plain letters/digits purely because the accented character next to it counts as a word boundary - and the code that stitches a row's own text back together with a codes recovered from that separate column crop was treating any such pattern-shaped fragment as proof the row already had its own code, so it quietly threw away the far more reliable, whitelisted-crop reading instead of using it. This build only lets a row's own reading pre-empt the crop-derived one when the two actually agree, which is the same bar already used elsewhere in this code to decide whether a reading can be trusted. One honest caveat: the image behind this specific row is genuinely very small, and even the whitelisted crop pass reads one character of it slightly wrong (an easy T/I mix-up at this resolution), so this particular row is a case worth a quick visual double-check against the real Ed post rather than a guaranteed-perfect read - a limit of the source image's resolution, not something more OCR passes can fix. (v1.3.44 removed a count-based rescue gate that could let an entire dropped table row go unrescued even though the codes and rows counts still looked balanced; v1.3.43 fixed three separate causes of FIT2102/FIT2109/FIT3162 codes never being found at all: Ed's lazy-loaded discussion list, course links being tagged with every enrolled unit's code instead of just their own, and Moodle's "My units" page needing far longer than the old wait budget to render; v1.3.42 added the debug logging that made all three visible instead of guesswork; v1.3.41 fixed the Ed fallback's flat 2-thread cap, a related but separate problem from the lazy-loading one above; v1.3.40 sent an opened Gmail message's screenshot-based code tables to OCR; v1.3.39 filled in the weeks Attendance's own UI can no longer show at all; v1.3.38 added the on-screen requested-vs-actual date range and fixed a debug-log write race; v1.3.37 fixed a flat Gmail thread cap sized for a single week's scan.)
