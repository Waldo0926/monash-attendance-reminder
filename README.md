...
> Current extension build: **v1.3.40** — found by opening a real, still-unresolved FIT2109 announcement by hand: its "Workshops:"/"Tutorials:" headings are ordinary text, but every attendance code sits inside two pasted screenshots of the code table, not text at all (confirmed live: 2 `<table>` elements with only 5 `<td>` total, next to 3 real `<img>` elements — the biggest two are the actual code tables). No amount of tuning the Gmail text-matching regex could ever have found those codes, because they were never text to begin with. This extension already had a whole OCR pipeline for exactly this situation, but it only ever ran on Ed discussion threads and Moodle section pages — an opened Gmail message with the identical screenshot never got sent to OCR at all. It now does. (v1.3.39 filled in the weeks Attendance's own UI can no longer show at all; v1.3.38 added the on-screen requested-vs-actual date range and fixed a debug-log write race; v1.3.37 fixed a flat Gmail thread cap sized for a single week's scan.)
