...
> Current extension build: **v1.3.41** — found by opening the real FIT2102 Ed course discussion list by hand: it doesn't keep one persistent "Attendance Codes" thread that gets edited in place, it posts a brand new thread every single week ("Week 8 Attendance Codes", "Week 7 International Students Attendance Codes", ...). The Ed fallback's thread picker had a flat cap of 2 threads total, sized for a normal single-week scan — reused unchanged by the full semester export, it meant only the one or two highest-numbered threads a course happened to post ever got opened, and every earlier week was silently left blank no matter how many discussion threads actually existed. It now works out which teaching weeks are still missing a code and opens the matching thread for each one, the same way the Gmail search was already split per week. (v1.3.40 sent an opened Gmail message's screenshot-based code tables to OCR, which the extension already did for Ed and Moodle pages but not Gmail; v1.3.39 filled in the weeks Attendance's own UI can no longer show at all; v1.3.38 added the on-screen requested-vs-actual date range and fixed a debug-log write race; v1.3.37 fixed a flat Gmail thread cap sized for a single week's scan.)
