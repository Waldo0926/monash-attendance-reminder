...
> Current extension build: **v1.3.39** — confirmed against the live site: the Attendance portal's own UI only ever renders roughly the last couple of weeks, no matter what date is requested in the URL. That is exactly why the full-semester export existed in the first place, but it was built entirely on top of that same portal read, so every earlier week was silently empty before Gmail/Ed/Moodle ever got a chance to look for a code. The export now infers each course's recurring (session, weekday, time) pattern from whatever week Attendance can still show, and projects it backward onto every earlier week from Week 1 — those filled-in rows are searched for codes exactly like a real row, and the CSV marks them "无法从Attendance查询" rather than a false "未签到", since Attendance was never actually asked to confirm them. (v1.3.38 added the on-screen requested-vs-actual date range and fixed a debug-log write race; v1.3.37 fixed a flat Gmail thread cap sized for a single week's scan.)
