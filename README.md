...
> Current extension build: **v1.3.46** — v1.3.45's fix turned out to be real but incomplete: it taught the code that stitches a row's text back together not to be fooled by a garbled fragment that merely looks code-shaped, but a real re-export showed the exact same FIT2102 Workshop 01 (Aug 18) row still coming up blank. Re-checking the real image against the real code found why: that same garbled reading ("CcQlTe" plus an accented character, still shaped like a 5-character code once uppercased for the reason v1.3.45 explained) was also satisfying an earlier check - "did the block pass already find as many codes as there are rows" - that decided whether to run the dedicated, whitelisted code-column crop pass at all. Because that fake code made the count look complete, the crop pass never ran in the first place, so v1.3.45's fix never got a chance to do its job. This build removes that earlier count check too and runs the code-column crop whenever the image is shaped like it has a code column, the same "trust the image's shape, not a count that a false positive can satisfy" principle v1.3.44 already applied elsewhere in this file. Verified end to end this time, not just read from the source: the real garbled OCR text plus the real crop reading were run through the actual, exported matching function from shared.js, and it now produces a code for this row instead of leaving it blank. One honest caveat carried over from v1.3.45: the image behind this specific row is genuinely tiny, and the whitelisted crop still reads one character of it slightly wrong (T as I) at that resolution, so this row is worth a quick visual check against the real Ed post rather than a guaranteed-perfect read. (v1.3.45 fixed pairRowsWithCodeColumn to require a row's own reading to actually agree with the crop-derived code before letting it block that code, which was correct but, as this version found, unreachable for this specific row; v1.3.44 removed a similar count-based rescue gate in the non-wideShort branch that could let an entire dropped table row go unrescued even though the codes and rows counts still looked balanced; v1.3.43 fixed three separate causes of FIT2102/FIT2109/FIT3162 codes never being found at all: Ed's lazy-loaded discussion list, course links being tagged with every enrolled unit's code instead of just their own, and Moodle's "My units" page needing far longer than the old wait budget to render; v1.3.42 added the debug logging that made all three visible instead of guesswork; v1.3.41 fixed the Ed fallback's flat 2-thread cap, a related but separate problem from the lazy-loading one above; v1.3.40 sent an opened Gmail message's screenshot-based code tables to OCR; v1.3.39 filled in the weeks Attendance's own UI can no longer show at all; v1.3.38 added the on-screen requested-vs-actual date range and fixed a debug-log write race; v1.3.37 fixed a flat Gmail thread cap sized for a single week's scan.)
