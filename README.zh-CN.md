...
> 当前扩展版本：**v1.3.21**
>
> v1.3.21 引入了新的最终核对流程：读取 Monash Attendance 时不再强制等待浏览器标签页进入 `complete` 状态，而是在页面 DOM 已可用后直接解析；Attendance 日期页面改为顺序读取，降低后台标签页长期停留在 `loading` 状态导致漏课的概率。同时，最终核对会优先使用已经发现的 Moodle 课程页面，再回退到 `My units` / Dashboard 查找课程。
...
8. 只有在确认页面勾选本人出席声明后，才允许提交所选记录到 Monash Attendance。
...
如果课程 URL 已经在前面的扫描中被发现，v1.3.21 会优先直接读取这些已知页面，不会因为 `My units` 页面长时间保持 `loading` 而阻塞整个最终核对流程。
...
- 某些 Monash 页面即使内容已经显示，Chrome 仍可能长期显示 `loading`。v1.3.21 已针对这种情况改为按可用 DOM 读取，但如果页面本身确实没有加载出内容，仍可能失败。
...
- 已签到 Attendance 记录保留；
- v1.3.21 最终核对模块 JavaScript 语法与导航策略。
