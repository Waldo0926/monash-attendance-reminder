...
> 当前扩展版本：**v1.3.44** —— v1.3.43 上线之后，一次真实的完整学期导出结果里，还是有少数几个 Ed 图片里的签到码没找到，而这次已经修好的翻页/匹配逻辑其实每次都精准打开了对的那个帖子，问题不在找不找得到帖子。对比文字识别各个阶段自己留下的记录和真实图片才看出来问题出在哪：在几张 FIT2102/FIT2109 的签到码表格图片上，整整一行——那一行的课程信息文字和它的签到码是一起消失的，不是单单丢了签到码那一格——在主识别流程里直接不见了，而同一张图里其他每一行都识别得很正常。原本设计用来补救这种情况的逻辑，是拿"识别出的签到码数量"去跟"识别出的行数"做对比，只有数量对不上的时候才会多跑一遍更仔细的识别；但一整行一起消失的时候，这两个数量是同步往下掉的，对比结果看起来仍然是"对得上"，所以这个补救逻辑压根就没被触发过。这个版本把这个数量对比拿掉了，改成只看图片本身的形状来决定要不要多跑几遍识别——只要图片高度看起来可能不止一行，就再完整识别一遍；只要图片形状看起来像是一张真的表格，就单独裁出签到码那一列再识别一遍。这样即使两边数量看起来是"对的"，一整行丢失的情况也还是能被重新捞回来。这个改动暂时还没有经过真实文字识别的实机验证（文字识别只能在浏览器扩展本身里跑），所以目前先当作"值得一试、但还没验证过"的修复，等下一次真实导出结果出来再确认。（v1.3.43 一次修了三个各自独立、导致 FIT2102/FIT2109/FIT3162 签到码彻底找不到的问题：Ed 讨论帖列表的懒加载、课程链接被同时打上所有已选课程代码而不是它自己的、以及 Moodle "My units" 页面需要比旧等待时间长得多才能渲染出来；v1.3.42 加的调试日志让上面这三个问题第一次变得看得见，而不是继续瞎猜；v1.3.41 修的是 Ed 备用路径里固定只挑 2 个帖子的数量上限问题，这是和上面"加载更多"那个问题相关但不同的另一个问题；v1.3.40 修复的是打开的 Gmail 邮件里，签到码表格是贴上去的截图时不会被送去做文字识别的问题；v1.3.39 补全了 Attendance 网站自己已经看不到的更早几周的课程记录；v1.3.38 在导出结果旁边加了"已请求范围 vs 实际返回范围"的对比提示，并修复了调试日志的一个写入竞争问题；v1.3.37 修复的是给单周扫描设计的固定 Gmail 邮件数量上限导致的签到码丢失问题。）
