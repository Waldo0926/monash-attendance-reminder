...
> 当前扩展版本：**v1.3.39** —— 已经对照真实网站确认过：Attendance 网站本身的页面不管请求哪个日期，最多也就只会显示最近一两周的记录，这是平台本身的限制。而"导出本学期历史记录"这个功能本来就是为了解决这个限制而做的，结果之前的实现却完全建立在同一个只能看最近几周的页面读取逻辑上，导致更早的每一周在 Gmail/Ed/Moodle 还没来得及去找签到码之前，课程记录本身就已经是空的了。现在导出功能会从 Attendance 还能看到的那一两周里，推算出每门课固定的（班次、星期、时间）规律，再把这个规律往前套用到 Week 1 到现在的每一周；这些补出来的记录会跟真实记录一样被拿去搜索签到码，导出的表格里也会显示"无法从Attendance查询（已超出可查询范围）"，而不是显示成"未签到"，因为 Attendance 压根就没机会去确认这些记录到底签没签到。（v1.3.38 增加了导出结果旁边"已请求范围 vs 实际返回范围"的对比提示，并修复了调试日志的一个写入竞争问题；v1.3.37 修复的是给单周扫描设计的固定 Gmail 邮件数量上限导致的签到码丢失问题。）
