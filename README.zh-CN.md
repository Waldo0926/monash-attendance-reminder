...
> 当前扩展版本：**v1.3.41** —— 亲自打开 FIT2102 在 Ed 上的讨论区列表才找到的问题：这门课根本不是维护一个反复编辑的"签到码"帖子，而是每周都单独发一个新帖子（"Week 8 Attendance Codes"、"Week 7 International Students Attendance Codes"……）。Ed 这条备用查找路径里，挑选帖子的逻辑原来固定只挑最多2个帖子，这个数量是按单周扫描设计的——学期导出功能直接复用了这段逻辑却没有改动，结果就是不管这门课实际发过多少周的帖子，永远只会打开周数最靠前的那一两个，更早的每一周都被这个数量上限直接挡在外面，查不到也不会有任何提示。现在会先算出这门课到底还缺哪几周的签到码，然后针对每一周分别去找对应的那个帖子并打开——和之前把 Gmail 搜索按周拆开的思路是一样的。（v1.3.40 修复的是打开的 Gmail 邮件里，签到码表格是贴上去的截图时不会被送去做文字识别的问题——插件对 Ed 和 Moodle 页面早就有这个截图识别功能，只是漏了 Gmail；v1.3.39 补全了 Attendance 网站自己已经看不到的更早几周的课程记录；v1.3.38 在导出结果旁边加了"已请求范围 vs 实际返回范围"的对比提示，并修复了调试日志的一个写入竞争问题；v1.3.37 修复的是给单周扫描设计的固定 Gmail 邮件数量上限导致的签到码丢失问题。）
