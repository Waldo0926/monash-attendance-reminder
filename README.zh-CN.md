...
> 当前扩展版本：**v1.3.23** —— 已签到课程现在会从第一阶段发现一直保留到最终核对；用于发现已完成课程的合成链接会和真正可填写的 `Entry.aspx` 链接严格区分；扩展重新加载后会补充修复已经打开的 Attendance 标签页；设置页的“保存后立即测试”也会等待最终核对完成后再显示结果。
...
已经完成的课程会使用稳定的班次身份保存，因此同一天有多条已签到记录时不会被去重成一条。第一阶段发现用的合成链接带有 `mah_completed=1`，最终核对只把它当作“已经完成”的证据；真正的 `Entry.aspx` 链接仍然代表待填写的 Attendance 记录。
...
10. 点击 **保存后立即测试**。v1.3.23 会先完成普通扫描，再执行最终 Attendance 核对，最后才显示统计结果。
...
- 某些 Monash 页面即使内容已经显示，Chrome 仍可能长期显示 `loading`；最终核对会直接读取已经可用的 DOM，但页面本身确实不可用时仍会失败。
...
当前回归覆盖包括 Gmail 多账号发现、严格班次匹配、OCR 打包、Moodle 回退、已签到记录保留、同日多条已签到记录、completed 与真实 `Entry.aspx` 的区分、最终核对流程以及防止重复提交。
