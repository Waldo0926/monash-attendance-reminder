...
> 当前扩展版本：**v1.3.40** —— 亲自打开了一封一直没解决的 FIT2109 邮件才找到的问题：邮件里"Workshops:"、"Tutorials:"这些标题是正常文字，但每一个签到码其实都在两张贴上去的表格截图里，压根就不是文字（实际检查过：邮件正文里只有2个`<table>`、一共5个`<td>`，但有3张真正的`<img>`图片，其中最大的两张就是签到码表格本身）。不管怎么调 Gmail 文字匹配的正则表达式都不可能找到这些码，因为它们从头到尾就不是文字。插件里其实早就有一整套专门处理这种"表格截图"的 OCR 识别流程，但之前只有 Ed 讨论区页面和 Moodle 周次页面会触发它——打开的 Gmail 邮件哪怕装的是一模一样的截图，也从来没被送去 OCR 过。现在修好了，Gmail 邮件也会走同样的 OCR 流程。（v1.3.39 补全了 Attendance 网站自己已经看不到的更早几周的课程记录；v1.3.38 在导出结果旁边加了"已请求范围 vs 实际返回范围"的对比提示，并修复了调试日志的一个写入竞争问题；v1.3.37 修复的是给单周扫描设计的固定 Gmail 邮件数量上限导致的签到码丢失问题。）
