...
11. 点击 **保存后立即测试**。这一步会在后台一次性完成普通扫描和最终 Attendance 核对，两步都跑完之后才会显示找到了多少节课和签到码，中途不用一直停留在设置页面。

之后如果你从仓库拉取了更新，记得回到 `chrome://extensions` 对这个未打包的扩展点一下重新加载。页面上显示的版本号应该和 `extension/manifest.json` 里的一致。

## 日常怎么用

装好之后，随时点一下扩展图标就能查一下有没有新代码。

![扩展弹窗，显示本周找到的代码数量，以及立即查码、查看确认页、设置三个按钮](docs/screenshots/popup.png)

1. 弹出的小窗口会显示本周已经找到多少个签到码（比如“4/5”），下面有三个按钮：**立即查码**、**查看确认页** 和 **设置**。
2. 点击 **立即查码** 会开始重新扫一遍最近的签到码，扫完之后会自动跳转到确认页面。
3. 确认页面里，每节课都单独一行，带勾选框、上课时间、当前状态（已签到、高可信、请核对等，跟前面“已完成签到的课程”那一节说的一致），以及找到的签到码。

![确认页面列出每节课的签到码、状态和出勤声明勾选框](docs/screenshots/confirm-page.png)

4. 勾选自己真正参加了的课程，再勾选最下面的出勤声明，点击 **确认并提交所选记录**，扩展就会自动把签到码填进去并提交到 Monash Attendance。
5. 到了你设置的提醒时间，点通知也会直接打开同一个确认页面，不用自己记得去查。

如果发现出勤率偏低，怀疑之前有课漏交了，可以打开扩展的 **设置** 页面，在“本学期历史记录”那一块点 **导出本学期历史记录**。它会重新扫一遍从 Week 1 到今天的每一节课，尽量把当时的签到码也找回来，跑完之后会导出成一份表格文件，可以自己留底或者发给任课老师申请补签。

![设置页面，红框标注了 Week 1 星期一日期、保存并启用按钮和导出本学期历史记录按钮](docs/screenshots/export-history.png)
