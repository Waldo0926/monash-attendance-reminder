...
> 当前扩展版本：**v1.3.42** —— v1.3.41 那个 Ed 帖子数量上限的修复并没有解决全部问题：用户自己实机跑了一次学期导出，FIT2102 和 FIT3162 这两门课的历史周依然是彻底空白的。问题出在 automaticSourceScans 的调试日志一直只覆盖了 Gmail 这一步——Ed 课程仪表盘到底扫到了什么、哪个 Ed 讨论帖被匹配给了哪门课、Moodle "My units" 页面本身、以及打开过的每一个 Moodle 周次页面，这些全都没有任何日志记录，所以从外部根本没法判断 Ed/Moodle 这条备用路径到底有没有真的跑起来、有没有匹配对课程、或者是不是打开了错误的页面。这个版本给上面每一步都加上了调试日志——找到了哪些 Ed 课程页面、判断缺了哪几周、实际打开了哪个帖子/哪个周次页面、抓到了什么内容——这样下一次真实导出就能看到 FIT2102 和 FIT3162 具体卡在哪一步，而不是继续猜。（v1.3.41 修的是 Ed 备用路径里固定只挑 2 个帖子的数量上限问题，这是和本次修复相关但不同的另一个问题；v1.3.40 修复的是打开的 Gmail 邮件里，签到码表格是贴上去的截图时不会被送去做文字识别的问题；v1.3.39 补全了 Attendance 网站自己已经看不到的更早几周的课程记录；v1.3.38 在导出结果旁边加了"已请求范围 vs 实际返回范围"的对比提示，并修复了调试日志的一个写入竞争问题；v1.3.37 修复的是给单周扫描设计的固定 Gmail 邮件数量上限导致的签到码丢失问题。）
