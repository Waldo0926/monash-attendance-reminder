...
> 当前扩展版本：**v1.3.38** —— 之前导出本学期历史记录经常从比 Week 1 晚很多的日期才开始，确认页的调试日志有时候还看起来完全是空的，没办法分清到底是"Week 1 日期填错了"还是"Attendance 网站本来就没返回那么早的记录"。现在导出功能会在任何校验报错之前先把 Week 1 的设置值记进日志，并且会把请求的日期范围和 Attendance 实际返回签到记录的日期范围做对比，直接显示在设置页导出结果旁边，不用再去翻调试日志才能看到。另外也修复了调试日志本身的一个真实的并发问题：如果导出本学期历史记录和每周自动扫描的定时任务同时在跑，两边都会读取再整体写回同一份日志列表，谁后写谁就把对方刚写的记录覆盖掉了；现在所有写入都会排队顺序执行，不会再丢失记录。（v1.3.37 修复的是学期导出因为固定的 Gmail 邮件数量上限（原本是给单周扫描设计的）导致大部分周次签到码丢失的问题。）
