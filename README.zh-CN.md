...
> 当前扩展版本：**v1.3.45** —— v1.3.44 上线之后，一次真实的重新导出证实它确实起作用了：之前找不到的 4 个 FIT2102/FIT2109 签到码里，有 3 个这次都正常识别出来了。剩下那一个（FIT2102 Workshop 01，8 月 18 日）是另一个不同的问题——直接打开真实的 Ed 帖子，把同一张图片手动跑一遍真实的文字识别代码才找到原因。这一行在 Ed 帖子里是单独发的一张只有 841×42 像素的极小图片，跟表格其余部分是分开的；即使经过扩展自己的放大处理，Tesseract 识别出来的签到码这一格也是一串明显的乱码（"CcQlTe"后面跟着一个带重音符号的字符），而专门为签到码这一列做的、带白名单限制的裁剪识别，从同一个位置正确识别出了一个真实的 5 位候选值。问题出在：那串乱码转成大写之后，碰巧凑出了 5 个连续的普通字母数字字符——纯粹是因为旁边那个带重音符号的字符被正则表达式的"单词边界"判断当成了分隔符，并不是识别真的读出了签到码。而负责把这一行的文字和列裁剪识别出来的签到码拼接回去的代码，把这种"长得像签到码"的乱码片段当成了"这一行本来就已经有签到码"的证据，于是悄悄地把那个更可靠、带白名单限制的裁剪识别结果给扔掉了。这个版本改成只有当这一行自己识别出来的结果，和列裁剪识别出来的结果真的一致时，才允许用自己的结果压过裁剪结果——这跟这段代码里其他地方判断"这个识别结果能不能信"用的是同一个标准。有一点需要老实说清楚：这一行背后的原图确实非常小，即使是带白名单限制的裁剪识别，也把其中一个字符识别错了一位（在这种分辨率下 T 和 I 很容易认错），所以这一行还是值得手动对照真实的 Ed 帖子核对一下，不能保证百分之百准确——这是原图分辨率本身的限制，不是再多跑几遍识别就能解决的。（v1.3.44 拿掉了一个基于数量对比的补救触发条件，那个条件在整行一起消失、签到码数量和行数依然同步对得上的时候，会让丢失的整行永远得不到补救；v1.3.43 一次修了三个各自独立、导致 FIT2102/FIT2109/FIT3162 签到码彻底找不到的问题：Ed 讨论帖列表的懒加载、课程链接被同时打上所有已选课程代码而不是它自己的、以及 Moodle "My units" 页面需要比旧等待时间长得多才能渲染出来；v1.3.42 加的调试日志让上面这三个问题第一次变得看得见，而不是继续瞎猜；v1.3.41 修的是 Ed 备用路径里固定只挑 2 个帖子的数量上限问题，这是和上面"加载更多"那个问题相关但不同的另一个问题；v1.3.40 修复的是打开的 Gmail 邮件里，签到码表格是贴上去的截图时不会被送去做文字识别的问题；v1.3.39 补全了 Attendance 网站自己已经看不到的更早几周的课程记录；v1.3.38 在导出结果旁边加了"已请求范围 vs 实际返回范围"的对比提示，并修复了调试日志的一个写入竞争问题；v1.3.37 修复的是给单周扫描设计的固定 Gmail 邮件数量上限导致的签到码丢失问题。）
