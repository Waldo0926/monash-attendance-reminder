...
> 当前扩展版本：**v1.3.43** —— v1.3.42 加的调试日志起作用了：一次真实的导出运行终于把 FIT2102 和 FIT3162 具体卡在哪里暴露了出来，而且这次发现其实是三个各自独立的真实 bug，不是一个。第一个：Ed 的讨论帖列表默认只渲染最近约 30 个帖子，要点一下"加载更多"才会加载更早的。FIT2102 第 1 到第 6 周的签到码帖子并不是被匹配逻辑漏掉了：而是这些帖子在抓取页面的那一刻根本就不存在于页面里。这个版本会在读取页面之前先点这个按钮（有次数上限；实机验证过某门课从 24 个帖子点到 278 个，第 1 到第 8 周全部都出现了）。第二个：把一个 Ed/Moodle 课程链接匹配到具体课程代码时，用的是从链接往上找若干层父元素拼出来的"周边文字"，但在真实的 Ed 仪表盘上，这段文字其实是整个侧边栏里所有已选课程名字拼在一起的大杂烩——结果每一个课程链接都被同时打上了所有请求的课程代码，而不是它自己真正对应的那一个。这个版本改成优先只用链接自身的文字去匹配，只有当链接文字本身完全没匹配到任何课程时，才退回去用那段更大范围的周边文字。第三个：Moodle 的 "My units" 页面实测光是把课程卡片渲染出来就要 17.4 秒，木身就已经超过了旧版 15 秒的等待上限，这正是 Moodle 备用路径为什么一门课都匹配不到的原因；等待上限现在改成了 45 秒。（v1.3.42 加的调试日志让上面这三个问题第一次变得看得见，而不是继续瞎猜；v1.3.41 修的是 Ed 备用路径里固定只挑 2 个帖子的数量上限问题，这是和上面"加载更多"那个问题相关但不同的另一个问题；v1.3.40 修复的是打开的 Gmail 邮件里，签到码表格是贴上去的截图时不会被送去做文字识别的问题；v1.3.39 补全了 Attendance 网站自己已经看不到的更早几周的课程记录；v1.3.38 在导出结果旁边加了"已请求范围 vs 实际返回范围"的对比提示，并修复了调试日志的一个写入竞争问题。）
