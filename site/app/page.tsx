...
export default function Home() {
  ...
  const activeCourses = useMemo(
    () => courses.filter((course) => course.enabled && course.name.trim() && course.url.trim() && course.sessions.length),
    [courses],
  );
      ...
      <section className="builder" aria-label="扩展设置器">
        <div className="form-column">
          ...
          <div className="course-list">
            {!courses.length && <div className="empty-course"><strong>还没有课程</strong><span>从“添加课程”开始，不会自动带入任何人的 FIT3162 / FIT2102 / FIT2109。</span></div>}
            {courses.map((course) => (
        ...
        <aside className="summary-column">
          <div className="summary-sticky">
            ...
            <div className="summary-block"><span className="summary-kicker">课程</span>{activeCourses.length ? activeCourses.map((course) => <div className="summary-row" key={course.id}><span className="check">✓</span><span>{course.name}</span><span>{course.source === "moodle" ? "Moodle" : "Ed"}</span></div>) : <div className="empty-state">尚未添加课程。</div>}</div>
